...
- **Nightpack Ambusher** — Flash lord that makes a 2/2 Wolf at every end step you didn't cast a spell — and casting nothing on your own turn is also the only *free* way to reach night without Tovolar (Unnatural Moonrise and The Celestus both cost mana).
...
- **Runebound Wolf** — `{3}{R}, {T}`: deals damage equal to your Wolf/Werewolf count **to an opponent** (not to creatures or planeswalkers). Four mana a shot is steep — it's in the deck mainly as a two-mana Wolf for the tribal count, with a non-combat damage outlet attached as a bonus.
...
- **Werewolf Pack Leader** — Draws whenever you attack with 6-plus total power, and can pump itself to a 5/3 non-Human trampler on demand. Single-faced, so it never transforms.
- **Wilderland Scrounger** *(new)* — A 3/6 **Wolf** — so it counts for Tovolar, Immerwolf, Nightpack Ambusher, Howlpack Resurgence, Runebound Wolf and both night-side lords — whose ferocious trigger puts a **permanent +1/+1 counter on every creature you control** each time it attacks while you *control* a 4-power creature — again, that creature needn't attack, so Scrounger can swing alone while a doubled Zopandrel holds the fort. A repeatable, permanent version of the one-shot pump spell it replaces, on a body that actually blocks, and it feeds Terrasymbiosis every attack.
...
### Lands (36)
- **Bristling Backwoods** — Tapped Gruul dual that pings an opponent on the way in. The weakest land in the deck and the first cut if you want another spell slot.
...
- **Mountain** (x13) — Red basics (63 available after reservations).
...
*10 nonbasic lands, of which **7 enter tapped** (Bristling Backwoods, Gongaga, Path of Ancestry, Raging Ravine, Rugged Highlands, Sheltered Thicket, Temple of Abandon). Command Tower, Exotic Orchard and Kessig Wolf Run are the untapped three.*
...
**Power level**: **Bracket 2** — a well-tuned casual deck. No tutors, no fast mana beyond Sol Ring, no combo, no stax, a five/seven-mana pair as the finisher and 7 of 10 nonbasics entering tapped. It beats precons and trades with other upgraded precons; it is not close to a bracket 4 table. Better in 1v1 than in a pod, where 2/2 Wolves get gang-blocked, Tovolar's draw requires *connecting*, and two planeswalkers absorb attacks badly — Zopandrel is the one card that is genuinely better with more opponents.
...
- If the collection expands, a one-sided sweeper, Rhythm of the Wild, or any untapped Gruul dual would close the three real gaps
...
---
Deck upgraded from the previous draft using cards in your Moxfield collection (moxfield_latest.csv & card_details.md), excluding cards reserved by Full Deployment, Squirreled Away and Dance of the Elements.
