...
3. `non_commander_cards.md` exists (also generated by `generate_cards_md.py`) — needed only for the "No Commander-precon cards" pool
4. The `Custom Decks/` directory exists for saving generated decks (create it if not; the script does not)
5. `reserved_decks.md` exists (hand-maintained list of decks that are currently assembled and off-limits). If it is missing, treat the reserved list as empty and mention that to the user — do not generate it.

If files 1–3 are missing, inform the user to run the card generation script first.
...
3. **Ask Setup Questions**: Before building the deck, ask the user both of the following questions at the same time (combine into a single message):
   ...
   **b) Card pool** — Which cards are available to build with. Offer all three options, in this order:
   - **No Commander-precon cards** — Draw only from cards not committed to any Commander precon **and** not in a reserved deck. The strictest option; keeps every Commander precon intact. Pool: `non_commander_cards.md` **minus** the reserved lists (see step 4 — `non_commander_cards.md` alone is *not* sufficient). Note this pool still includes non-Commander precons and Secret Lair drops, which are always considered free.
   - **Avoid reserved decks** *(recommended default)* — Draw from the whole collection except the decks currently assembled and sleeved, as listed in `reserved_decks.md`. Commander precons that are **not** reserved are fair game. Pool: `moxfield_latest.csv` minus the reserved lists.
   - **Full collection** — Draw from every owned card (`moxfield_latest.csv`), including cards currently sitting in an assembled deck. Widest possible pool; assumes you're happy to cannibalise anything.

   The reserved subtraction applies to the **first two** options — a card in an assembled deck is physically unavailable regardless of which precon it originally came from. Only "Full collection" skips it.

   When reading out the middle option, name the decks currently in `reserved_decks.md` so the user can see what would be off-limits, and mention that spare copies of a reserved card are still usable.

   Use both answers to guide card selection throughout the rest of the process.

4. **Read Card Collection**:

   **Primary source (all modes)**: Read `moxfield_latest.csv` to get the list of all owned cards
   ...
   **If "No Commander-precon cards" mode**:
   - Start from `non_commander_cards.md` — it lists every card not reserved by a Commander precon, with available copy counts
     - Columns: Card, Copies, CMC, Type, Color Identity, Notes
     - The `Copies` column is the quantity that precon reservation leaves free
     - Cards from other precons (Foundations Beginner Box, Secret Lair drops, etc.) are already included here
   - **Then apply the reserved-deck subtraction on top** (see the next block). This is required, not optional: `non_commander_cards.md` knows nothing about `reserved_decks.md`, so it happily lists cards that are physically sitting in an assembled custom deck. At the time of writing, 13 of its entries are copies that live in Full Deployment — 12 of them with **zero** free copies, including Swiftfoot Boots and The Ozolith. Taking the `Copies` column at face value will put unplayable cards in the deck.
     ```
     available[card] = min( Copies in non_commander_cards.md,
                            total in moxfield_latest.csv − reserved copies )
     ```
   - Cross-reference `moxfield_latest.csv` if you need edition or additional quantity detail not in the file

   **If "Avoid reserved decks" mode** (and for the reserved half of the mode above):
   - Read `reserved_decks.md` for the list of currently assembled decks and the path to each one's card list. If the table is empty, this mode is identical to Full collection — say so and continue.
   - Build the eligible pool by **subtracting copies, not names**:
     ```
     available[card] = total count in moxfield_latest.csv
                     − sum of counts across every reserved list
     ```
     Keep cards where `available > 0`, and never use more copies than `available`. A card with three copies of which one is reserved still has two available — this is the whole point of the mode.
   - **Resolve alt names before subtracting.** Reserved custom-deck exports use alt names where they exist (`Air Shoes` → Swiftfoot Boots, `Power Sneakers` → Lightning Greaves). Build a **reverse** index to do this: `moxfield_cards.md` is keyed by *real* name, with the alt in the `Alt Name(s)` column as `Air Shoes (SonictheHedgehog TurboGear)` — strip the trailing ` (PreconName)` annotation, then map alt → real. Skipping this fails *silently*: the reserved line matches no collection row, nothing is subtracted, and the card looks available.
   - Strip set codes, collector numbers and foil markers from precon lines (`1 Sol Ring (BLC) 129`, `1 Hazel of the Rootbloom (BLC) 2 *F*`). Sum basic lands across the multiple rows precon files split them over.
   - For a reserved *custom* deck, use its `Custom Decks/Moxfield/*.txt` export as the authoritative list, not its `.md`. If no `.txt` export exists, parse the `.md` decklist sections instead and say that you did — the `.md` is prose-formatted and easier to misparse.
   - A reserved *Commander precon* uses its `Precons/Commander Precons/*.txt` list, which is what the precon **shipped with**. If the user has since upgraded that precon in paper, the list is stale — flag this if the deck matters to the build rather than assuming it is accurate.
   - **Verify the parse before building**: every reserved card name should resolve to a name present in `moxfield_latest.csv`. Any that doesn't indicates an alt-name or parsing failure — fix it rather than proceeding, or the deck will include cards the user cannot physically play.
   - Do **not** use `non_commander_cards.md` in this mode — it excludes all Commander precons, which is the stricter question.
   - In the finished deck file, record which pool was used and note in the "Cards Removed from Original" reasons where a cut was forced by a reservation rather than by card quality.

   **If "Full collection" mode**: use `moxfield_latest.csv` counts directly with no subtraction.
...
9. **Lore Summary**: Before writing the file, write a lore blurb of **maximum 230 characters** (including spaces) that gives the deck a personality and narrative identity. This goes directly after the Overview section.
...
10. **Suggested Deck Names**: Before writing the file, generate 4 creative name candidates for the deck. Include them in a "Suggested Deck Names" section near the bottom of the file (after Next Steps, before the version footer). Use the file name as a working title (e.g., `Bria_Riptide_Rogue_Deck`) until the user picks one.
...
11. **Output Format**: Create a markdown file in `Custom Decks/` with this structure (see existing deck for reference):

**Heading levels are not decorative** — use `#`, `##` and `###` exactly as shown. The verification checks in this skill and in `CLAUDE.md` parse `### Section (N)` headings to rebuild the decklist, and bare text headings break them.

**Section order is fixed** and matches the section placement priority below: Commander → Creatures → Enchantments → Artifacts & Mana → Instants → Sorceries → Planeswalkers → Lands. Omit any section that would be empty. **Instants and Sorceries are always separate sections, never a combined "Instants & Sorceries"** — the type-line check depends on it.
...
- **Card pool**: [No Commander-precon cards / Avoid reserved decks (name them) / Full collection]
...
---
Deck created from cards in your moxfield collection (moxfield_latest.csv & card_details.md)[, excluding cards reserved by <reserved decks> — only if a reserved pool was used].
...
14. **Moxfield Export File**: After writing the deck file, create a separate plain-text export file in the `Moxfield/` folder for easy import into Moxfield.
...
5. **Ask whether to add the deck to `reserved_decks.md`** if it isn't already listed — locking usually means the deck is about to be built in paper. Nothing else maintains that file, and a stale entry (or a missing one) silently corrupts every later build's card pool. Add the row only if the user confirms; likewise offer to remove a row when a deck is dismantled.
...
- **Insufficient cards**: If collection has <100 cards matching criteria, explain what's available and suggest:
  ...
  - Loosening the card pool one step (No Commander-precon cards → Avoid reserved decks → Full collection), naming which specific cards that would unlock
- **Reserved deck gutted the build**: If "Avoid reserved decks" removes a large number of cards from an existing deck being improved, say so up front with the count, list what was lost and why, and continue with the best available replacements. Do not silently substitute weaker cards — the user needs to know which cuts were forced by availability rather than by power, so they can decide whether to dismantle a reserved deck instead.
- **`reserved_decks.md` missing or empty**: Treat the reserved list as empty, tell the user, and proceed as Full collection for that option.
